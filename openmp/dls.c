...
void dls_( int *threads, int *len,  double *a, double *b, double *x ){
    ...
    if ( ! strictlyDiagonallyDominant( N, a ) ) {
        ...
        for (k=0;k<N-1;k++) {
            ...
            *(p+k) = iPivot;
            if ( *(a+k*N+k) != ZERO ) {
                #pragma openmp parallel for shared(N) private(rows, rows2) 
        ...
        *(b+N-1) = *(b+N-1) / *(a+N*(N-1)+(N-1));
        #pragma omp parlallel for sahred(N) private(i,j, tmp)
        for (i=N-2;i>=0;i--){
            tmp = 0.0;
            for (j=i+1;j<N;j++) {
                tmp = tmp + *(a+i*N+j) * *(b+j);
        ...
        #pragma omp parallel for shared(N) private(k,rows,rows2)
        for (k=0; k<N-1; k++) {
            for (rows=k+1;rows<N;rows++) {
                *(a+rows*N+k) = *(a+rows*N+k) / *(a+k*N+k);
        ...
        *(b+N-1) = *(b+N-1) / *(a+N*(N-1)+(N-1));
        #pragma omp paralell for shared(N) private(i,j,tmp)
        for (i=N-2;i>=0;i--){
            tmp = 0.0;
            for (j=i+1;j<N;j++) {
                tmp = tmp + *(a+i*N+j) * *(b+j);
